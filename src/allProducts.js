...
async function fetchProducts(sortBy = "name", direction = "asc", page) {
  ...
  data.forEach((item) => {
    ...
    productCard.innerHTML = `<div
        class="rounded-xl  capitalize relative h-full grid grid-rows-2   "
      >
       <div class=" flex items-center justify-center overflow-hidden row-span-3 "><a href="/shopwill-store/viewProduct.html?id=${item.id}"> <img src="${prodImage}" class=" hover:scale-103 transition-all h-full w-full duration-300 overflow-hidden" /></a></div>
